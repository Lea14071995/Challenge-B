...
###Step 1
We have two machine learning technique in order to make prediction on a non parametric model:

**Random forest** :  
It's a machine learning method that give a probability of each decision of a decisional tree based on the existing data.
...
**Kernel estimation**: 
This method estimates the random variable's probability density. It generalize. It weighted average estimators that use kernel functions as weights. 

It's an instance-based learning, that mean that it's a family of learning algorithms which "compares new problem instances with instances seen in training". The advantage of kernel estimation is that it is able to adapt its model to unseen data rather than using a fixed set of
parameters. 
...
```
Here before implementing the random forest method, we get ride of the missing value. Then we converted our observations in numerics features/factors to be able to do a random forest. 

##Step 3
```{r, echo=TRUE, warning=FALSE}
test <- read_csv("C:/Users/geraldine/rprog/test.csv")
test.rf <- randomForest(SaleCondition ~ ., data=test)
...
We now have 2 models: ll.fit.lowflex and ll.fit.highflex that we plott using the predictions on only the training data. 
...
In this prediction we can see that the high flexibility prediction now has a bigger bias and so it's the low flexibility prediction which become more interesting. 
...
We have: 
blue curve for test data
orange curve for the training data
